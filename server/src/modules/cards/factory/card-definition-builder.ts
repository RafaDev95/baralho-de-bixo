...
function validateEnergyCost(energyCost: number): void {
  if (energyCost < 0 || energyCost > 10 || !Number.isInteger(energyCost)) {
    throw new Error(
      `Invalid energy cost: ${energyCost}. Must be an integer between 0 and 10.`
    );
  }
}
...
function validateCreatureStats(power: number, toughness: number): void {
  ...
  if (toughness < 0 || !Number.isInteger(toughness)) {
    throw new Error(
      `Invalid toughness: ${toughness}. Must be a non-negative integer.`
    );
  }
}
...
): card is CreatureCardDefinition {
  return card.type === 'creature';
}

export function isSpellCard(
  card: TypedCardDefinition
): card is SpellCardDefinition {
  return card.type === 'spell';
}

export function isEnchantmentCard(
  card: TypedCardDefinition
): card is EnchantmentCardDefinition {
  return card.type === 'enchantment';
}

export function isArtifactCard(
  card: TypedCardDefinition
): card is ArtifactCardDefinition {
