...
import { cardsTable, deckCards, decksTable } from '@/db/schemas';
import type { AppRouteHandler } from '@/lib/types';
import { MIN_DECK_SIZE } from '@/utils/constants';
...
export const create: AppRouteHandler<Create> = async (c) => {
  ...
  const deckWithCards = await db.query.decksTable.findFirst({
    where: (decksTable, { eq }) => eq(decksTable.id, result.id),
    with: {
      cards: {
        columns: {
          deckId: true,
  ...
  return c.json(deckWithCards, HttpStatusCodes.CREATED);
};
